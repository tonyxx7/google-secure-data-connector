...
public class SecureDataConnection {
  ...
  public void connect() throws IOException, ConnectionException {
    ...
    if (sslSocketFactory != null) { // We have an SSL socket factory, use it.
      ...
      // wait for 5 sec to connect. is that too long?
      try {
        clientSocket.connect(new InetSocketAddress(localConf.getSdcServerHost(),
            localConf.getSdcServerPort()), 5000);
    ...
    if (sshdProcess == null) {
      ...
      throw new ConnectionException("couldn't start SSH. exiting");
    }
  ...
  /**
   * A thread to take data from the given stream and write it to the log file provided.
   */
