...
import com.google.securelink.util.ConnectionException;
...
/**
 * Entry point class for starting the Secure Link Client.  There are three components to the
 * Secure Link Client: 
 * 
 * 1) The "secure link connection" to the server which provides the transport from the server
 * side back to the client.  see {@link SecureLinkConnection}
 * 2) The Socks 5 proxy which provides the network firewall to incoming network connections through
 * the secure link transport. see {@link JsocksStarter}
...
public class Client {
  ...
  public void startUp() throws IOException, ConnectionException {
    ...
    jsocksStarter.start(); 
    SecureLinkConnection secureLinkConnection = new SecureLinkConnection(clientConf, 
        sslSocketFactory);
    secureLinkConnection.connect();
  }
  
  /**
   * Entry point for the Secure Link Client binary.  Sets up logging, parses flags and
   * creates ClientConf.
   * 
   * @param args
   */
