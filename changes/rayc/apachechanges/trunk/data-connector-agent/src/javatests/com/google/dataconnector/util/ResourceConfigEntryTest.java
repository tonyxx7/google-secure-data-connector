...
public class ResourceConfigEntryTest extends TestCase {
  ...
  public void testEquals() throws ResourceConfigException {
    // Valid case.  
  ...
  public void testSetAllowedEntities() throws ResourceConfigException {
  ...
  public void testParseResourceRule() {
    ...
    // invalid case - no allowed entities specified
    try {
      ClientConf.parseResourceRule("rule ");
    } catch (ResourceConfigException e) {
      assertTrue(e.getMessage().startsWith("Expected but not found"));
    }
    // invalid case - no rule specified
    try {
      ClientConf.parseResourceRule("user1,group1 ");
    } catch (ResourceConfigException e) {
      assertTrue(e.getMessage().startsWith("Expected but not found"));
    }
    // invalid case - empty line
    try {
      ClientConf.parseResourceRule("   ");
    } catch (ResourceConfigException e) {
      assertTrue(e.getMessage().startsWith("Expected but not found"));
    }
    // invalid case - empty line
