...
import com.google.pinky.ServiceApiStarter;
...
public class Client {
  ...
  private static ServiceApiStarter serviceApiStarter;
  ...
  public void startUp() throws IOException, ConnectionException {
    ...
    SecureDataConnection secureDataConnection = new SecureDataConnection(clientConf, 
        sslSocketFactory);
    if (clientConf.getServiceApiPort() > 0) {
      ClientHealth clientHealth = new ClientHealth(secureDataConnection);
      serviceApiStarter.setServiceHandler(clientHealth);
      serviceApiStarter.start(); // Started here due to SecureLinkConnection#connect blocking.
    }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // Ready the ServiceApi system by seeding it with command line arguments.
      if (clientConf.getServiceApiPort() > 0) {
        serviceApiStarter = new ServiceApiStarter(args);
      }
