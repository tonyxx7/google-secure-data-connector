...
import com.google.dataconnector.util.AtomicLongCounter;
...
import com.google.dataconnector.util.Counter;
import com.google.dataconnector.util.CountingInputStream;
import com.google.dataconnector.util.CountingOutputStream;
...
public class SecureDataConnection {
  ...
  private Counter inputStreamCounter;
  private Counter outputStreamCounter;
  ...
  public SecureDataConnection(final ClientConf clientConf,
    ...
    inputStreamCounter = new AtomicLongCounter();
    outputStreamCounter = new AtomicLongCounter();
  }

  public long getCurrentTransferCount() {
    return inputStreamCounter.get() + outputStreamCounter.get();
  }
  ...
  public void connect() throws IOException, ConnectionException {
    ...
    log.info("Connecting SSHD to existing streams");

    ConnectStreams connectSshdOutput = new ConnectStreams(p.getInputStream(),
        new CountingOutputStream(clientSocket.getOutputStream(), new Counter[] {outputStreamCounter}),
        "sshdOut");
    ConnectStreams connectSshdInput = new ConnectStreams(new CountingInputStream(
        clientSocket.getInputStream(), new Counter[] {inputStreamCounter}), p.getOutputStream(), "sshdIn");
