 ...
 * </p>
 * usage: KeyStore ks = KeyStore.getInstance("JKS"); File kf = new
 * File("keystore"); ks.load(new FileInputStream(kf),
...
public class SSLByteChannel implements ByteChannel {
   ...
   * @param engine A SSLEngine instance that will remember SSL current context.
   ...
   * 
   * @throws java.io.IOException May be raised by close operation on wrapped
   *         byte channel
   */
  ...
  /**
   * Fill the given buffer with some bytes and return the number of bytes added
   * in the buffer.<br>
   ...
   * so be careful with buffer position, limit, ... Check corresponding javadoc.
   * 
  ...
  private SSLEngineResult wrap() throws IOException, SSLException {
    ...
    while (outNetData.hasRemaining())
      wrappedChannel.write(outNetData); // TODO : To be enhanced (potential
                                        // deadlock ?)
    outNetData.compact();
