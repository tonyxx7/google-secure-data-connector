...
/**
 * Class to manage the HTTP message headers and body content, which could be
 * anything at this point This class should be extended by HttpMessageRequest
 * and HttpMessageResponse classes and never used directly
 */
public abstract class HttpMessage {
  ...
  protected static Logger logger = Logger.getLogger(HttpMessage.class);
  ...
  public void setBodyContent(final byte[] b) {
    body = b.clone();
    updateContentLength();
  }
  ...
  public ByteArrayInputStream getBodyContentStream() {
  ...
  public void setToHost(String h) {
    toHost = h;
  }
  ...
  public void setHeaders(Vector v) {
    for (int i = 0; i < v.size(); i++) {
       ...
       * while( st.hasMoreTokens() ) { addHeader(flag, st.nextToken() ); }
       */
    }
  }

  /**
   * Return a String with the headers printed in their RFC compliant final
   * version
   */
  ...
  public int getContentLength() {
    if (!isContentLengthSet()) return -1;
    return new Integer((String) ((Vector) headers.get(HEADER_CONTENT_LENGTH))
        .elementAt(0));
  }
