...
public class SecureDataConnection {
  ...
  private static final String SDC_CONNECT_FALSE = "SDC_CONNECT_FALSE";
  private static final String SDC_CONNECT_TRUE = "SDC_CONNECT_TRUE";
  ...
  public SecureDataConnection(LocalConf localConf, List<ResourceRule> resourceRules,
      ...
      JsocksStarter jsocksStarter, FileUtil fileUtil) {
  ...
  public void connect() throws IOException, ConnectionException {
    ...
    fileUtil.deleteFile(localConf.getSdcStatusFile());
    fileUtil.writeFile(localConf.getSdcStatusFile(), SDC_CONNECT_FALSE);
    ...
    fileUtil.writeFile(localConf.getSdcStatusFile(), SDC_CONNECT_TRUE);
  ...
  public static class KillProcessShutdownHook extends Thread {
    ...
    @Override
    public void run() {
      // We print a message for the script wrapper to pick up and use indicating our success
      // As we do not have reliable exit codes.
      proc.destroy();
    }
  }
